...
RUN apt-get update && apt-get install -y sane-utils curl udev imagemagick unpaper
...
RUN sed -i "s/\$SCANIMAGE \$OPT/\$SCANIMAGE \$OPT \&\& unpaper --no-mask-center \$OUTPUT.tif \$OUTPUT.ppm \&\& convert \$OUTPUT.ppm \$OUTPUT.png \&\& rm \$OUTPUT.tif \$OUTPUT.ppm/g" /opt/brother/scanner/brscan-skey/script/scantofile.sh
